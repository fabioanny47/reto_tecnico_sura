...
class FindPolicyByIdUseCaseTest {
    ...
    @BeforeEach
    void setUp() {
        findPolicyByIdUseCase = new FindPolicyByIdUseCase(policyRepository);
    }
    ...
    void execute() {
        ...
        when(policyRepository.findById(any(UUID.class))).thenReturn(Mono.just(getPolicy()));
        Mono<Policy> result = findPolicyByIdUseCase.execute(UUID.randomUUID());
